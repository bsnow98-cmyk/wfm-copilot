...
| 3 | **Break / activity move** ✅ **shipped** | Move a break or swap an activity for one agent | Medium | S | Shipped: `preview_break_move` tool — resolves the agent's next break, builds the two-segment change set (free old slot → place break later/earlier), delegates to `preview_schedule_change`. Reuses `POST /schedules/apply` entirely — no new write path. |
...
| 5 | **`apply_staffing_target`** ✅ **shipped** | Change an SL/ASA target → recompute staffing | Medium | M | First **async** surface: migration `0021_staffing_target_change_log.sql`, `preview_staffing_target` tool (shows current→proposed targets + est. peak impact), `POST /staffing/targets/apply` returns **202** + kicks off a `BackgroundTask` recompute (Erlang C, same pattern as the schedule solver; swept by `job_sweeper`), `GET /{log_id}` to poll, `/{log_id}/undo`. Recomputes the **single current scenario in place** (dashboard joins have no `staffing_id` predicate, so a parallel scenario would fan out). Whole-horizon targets only — **per-window targets = v2** (needs a per-interval-target schema). |
| 6 | **Shift creation / roster add** ✅ **shipped** | Create a new shift / add an agent to a day | Lower | L | Shipped behind RBAC (below): `preview_create_shift` tool builds a work/lunch/work shift and rides `POST /schedules/apply`, now gated to **wfm_manager+**. Audited as `applied_by=<user>`. Biggest blast radius — correctly the most-restricted gate. |

**Gating dependency for breadth — ✅ RBAC shipped.** Identity rides the existing shared-password gate: the Basic-Auth *username* (previously ignored) is now resolved against a `users` table (migration `0022`) for a role. Four roles — `viewer` (read), `analyst` (+forecast/staffing/leave), `wfm_manager` (+offers, schedule, break move, shift creation), `admin` (all). `require_role(...)` gates every write router; writes record `applied_by=<username>`. Frontend identity picker sets a `wfm_user` cookie the proxy forwards as the Basic username; `GET /me` + `/users` back it. The shared password remains the only secret (no per-user credentials) — identity + authorization for a demo, not a full auth system. *Autonomous* execution (no human click) remains explicitly out of scope (see below).
