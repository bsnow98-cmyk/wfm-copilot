...
|---|---|---|---|---|---|
| **1** | **Leave approval** | Approve / deny a pending leave request from chat | **High** | **S–M** | Specced below. `leave_requests` already has `status`/`decided_at`/`decided_by`/`decision_note`; `recommend_leave_approval` + `check_leave_feasibility` already produce the verdict + SL math. Pure reuse. |
| 2 | OT / VTO offer publish | Post an OT or VTO offer to a target group | High | M | `recommend_ot` / `recommend_vto` exist. Needs an `offers` table + acceptance model (out of scope for v1 of this surface — publish only, no agent-accept loop yet). |
